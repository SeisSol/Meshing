...
void testCommunication(counter_t* edgeCut, size_t messageSize, int numTimesteps) {
  ...
  double time_min = std::numeric_limits<double>::max(),
         time_max = std::numeric_limits<double>::min(), time_avg;
  int rank, numRanks;
  ...
  for (unsigned t = 0; t < numTimesteps; ++t) {
    ...
    for (unsigned r = 0; r < numRanks; ++r) {
      if (edgeCut[r] > 0) {
        MPI_Isend(copy + e * messageSize,
                  edgeCut[r] * messageSize,
                  MPI_BYTE,
                  r,
                  0,
                  MPI_COMM_WORLD,
                  requests + 2 * n);
        MPI_Irecv(ghost + e * messageSize,
                  edgeCut[r] * messageSize,
                  MPI_BYTE,
                  r,
                  0,
  ...
  printf("%4d %10.2lf %10.2lf %10.2lf %10.2lf\n",
         ...
         time_min,
         time_max,
         E * messageSize / 1.048576 / time_avg);
