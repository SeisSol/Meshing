...
Graph::Graph(unsigned numberOfNodes) : m_numberOfNodes(numberOfNodes) {
...
void Graph::printDOT(const std::string& filename) {
  ...
  for (unsigned from = 0; from < m_numberOfNodes; ++from) {
    for (unsigned to = from + 1; to < m_numberOfNodes; ++to) {
      maxEdgeCut = std::max(
          maxEdgeCut, m_edges[from * m_numberOfNodes + to] + m_edges[to * m_numberOfNodes + from]);
    }
  }
  for (unsigned from = 0; from < m_numberOfNodes; ++from) {
    counter_t edgeCut = 0;
    ...
    for (unsigned to = from + 1; to < m_numberOfNodes; ++to) {
      counter_t edgeCut =
          m_edges[from * m_numberOfNodes + to] + m_edges[to * m_numberOfNodes + from];
      if (edgeCut > 0) {
...
void Graph::printMatrix(const std::string& filename) {
