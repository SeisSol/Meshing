...
Mesh::Mesh(const std::string& fileName) {
  ...
  //logInfo() << "Evenly distribute" << numPartitionCells << "elements onto" << numRanks << "ranks...";
    ...
    for (size_t side = 0; side < 4; side++) {
      ...
      if (decodeBC(side) == 0 || decodeBC(side) == 3) {
        if (neighbors[side] < 0 && !face.isShared()) {
          logError() << "Element" << cell.gid() << ", side" << side << " has a regular or DR boundary condition, but the neighborig element doesn't exist";
        }
      }
      // absorbing or free surface boundaries must not have neighbor elements:
      else {
        if (neighbors[side] >= 0 || face.isShared()) {
          logError() << "Element" << cell.gid() << ", side" << side << " has an absorbing or free surface boundary conditin, but the neighborig element is not flagged -1";
        }
      }
    }
  }
